...
class Mix(mi.Texture):
    ...
    def __init__(self, props):
        ...
        if self.clamp_factor:
            self.factor = dr.clip(self.factor, 0.0, 1.0)
    ...
    def eval(self, si, active):
        ...
        return mi.UnpolarizedSpectrum(self.process(val_a, val_b))

    def eval_1(self, si, active):
        val_a = self.a.eval_1(si, active)
        val_b = self.b.eval_1(si, active)
        return mi.Float(self.process(val_a, val_b))

    def eval_3(self, si, active):
        val_a = self.a.eval_3(si, active)
        val_b = self.b.eval_3(si, active)
        return mi.Color3f(self.process(val_a, val_b))

    def process(self, val_a, val_b):
        # print("computing value of mix color")
        #TODO add remaining blend types
        if self.blend_type == blend_type_mix:
            result = (1 - self.factor) * val_a + self.factor * val_b
        else:
